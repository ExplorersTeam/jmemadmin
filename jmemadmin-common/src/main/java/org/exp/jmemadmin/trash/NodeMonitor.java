...
public class NodeMonitor {
	...
	public void connectZookeeper() throws IOException, KeeperException, InterruptedException {
		zk = new ZooKeeper(CommonConfigs.getZKQuorum(), 5000, new Watcher() //注册全局默认watcher
		{
			@Override
			public void process(WatchedEvent event) {
				LOG.info("**********event.getPath() is : " + event.getPath());
				if((event.getType() == EventType.NodeChildrenChanged) && (CommonConfigs.getZNodeRoot().equals(event.getPath()))) {
	...
	private void updateServerList() throws KeeperException, InterruptedException, UnsupportedEncodingException {
		...
		List<String> subList = zk.getChildren(CommonConfigs.getZNodeRoot(), true);//watcher注册后，只能监听事件一次，参数true表示继续使用默认watcher监听事件
		for(String subNode : subList) {
			byte[] nodeData = zk.getData(CommonConfigs.getZNodeRoot(), false, null);//获取节点数据
